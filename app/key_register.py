...
class KeyRegister:
    def __init__(self, on_press_fn=None, on_release_fn=None) -> None:
        self.listener = keyboard.Listener(on_press=on_press_fn, on_release=on_release_fn)
        self.listener.start()
...
def main():
    keyreg = KeyRegister()
    listener = keyboard.Listener(on_press=keyreg.on_press, on_release=keyreg.on_release)
    listener.start()
    listener.join()
